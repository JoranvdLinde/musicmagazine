...
$stmt = $pdo->prepare("SELECT u.*, p.profile_image, p.bio 
    FROM music_users u 
    LEFT JOIN music_profiles p ON u.id = p.user_id 
    WHERE u.id = ?");
$stmt->execute([$userId]);
$userProfile = $stmt->fetch();

// Get user's articles
$stmt = $pdo->prepare("SELECT * FROM magazine_articles 
    WHERE user_id = ? 
...
<head>
    ...
    <title><?php echo htmlspecialchars($userProfile['username']); ?>'s Profile - Music Today</title>
...
<body class="bg-magazine-dark text-white font-roboto">
    ...
    <main class="lg:ml-64 p-4 lg:p-8 pb-20 lg:pb-8">
        <!-- Profile Header -->
        <div class="bg-magazine-card rounded-2xl p-8 mb-8">
            <div class="flex items-center space-x-6">
                <img src="<?php echo htmlspecialchars($userProfile['profile_image'] ?? 'default-avatar.jpg'); ?>"
                     alt="Profile"
                     class="w-24 h-24 rounded-full border-4 border-magazine-accent">
                <div>
                    <h1 class="text-3xl font-bold font-lato mb-2">
                        <?php echo htmlspecialchars($userProfile['username']); ?>
                    </h1>
                    <p class="text-gray-400">
                        <?php echo htmlspecialchars($userProfile['bio'] ?? 'No bio available'); ?>
                    </p>
                </div>
            </div>
        </div>
        ...
        <!-- User's Articles -->
        <h2 class="text-2xl font-bold font-lato mb-6">Articles by <?php echo htmlspecialchars($userProfile['username']); ?></h2>
                ...
                <article class="bg-magazine-card rounded-xl overflow-hidden shadow-lg hover:shadow-2xl transition duration-300 cursor-pointer"
                    ...
                    <div class="p-6">
                        ...
                        <div class="flex justify-between items-center text-sm text-gray-400">
                            <span><i class="far fa-clock mr-2"></i><?php echo date('M d, Y', strtotime($article['created_at'])); ?></span>
                            <span><i class="far fa-file-alt mr-2"></i><?php echo ceil(str_word_count($article['content']) / 200); ?> min read</span>
                        </div>
                    </div>
                </article>
